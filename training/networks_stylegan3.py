...
class MappingNetwork(torch.nn.Module):
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False):
        ...
        if self.c_dim > 0:
            ...
            # img = self.InputG(x, y, truncation_psi=truncation_psi, noise_mode=noise_mode)
            # x = (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)

            # x = self.transform(x)

            # x = torch.reshape(x, (x.shape[0],16*32))
            # if self.c_dim > 0:
            #     x = torch.cat([x, y], dim=1) if x is not None else y
