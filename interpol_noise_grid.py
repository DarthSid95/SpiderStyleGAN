...
def generate_images(
    ...
    impath = outdir+'/Interpol_NoiseImages_Supplementary'
    midpath = outdir+'/Interpol_NoiseMidImages_Supplementary'
    ...
    # Labels.
    num_interps = 7
    label = torch.zeros([(num_interps+1)**2, G.c_dim], device=device)
    if G.c_dim != 0:
    ...
    for i in range(len(seeds)-3):
        ...
        # z_br_img = InputG(z=z_bl, c=label, truncation_psi=truncation_psi, noise_mode=noise_mode)
        ...
        #### Col 1: 
        stack = np.vstack([z_tl.cpu().numpy(), z_bl.cpu().numpy()]) 
        linfit = interp1d([1,num_interps+1], stack, axis=0)
        col1_interp_latentsPart = linfit(list(range(1,num_interps+1)))
        col1_interp_latents = np.vstack([col1_interp_latentsPart, z_bl.cpu().numpy()]) 

        #### Col n: 
        stack = np.vstack([z_tr.cpu().numpy(), z_br.cpu().numpy()]) 
        linfit = interp1d([1,num_interps+1], stack, axis=0)
        coln_interp_latentsPart = linfit(list(range(1,num_interps+1)))
        coln_interp_latents = np.vstack([coln_interp_latentsPart, z_br.cpu().numpy()]) 
        ...
        all_z = torch.from_numpy(all_interps)
        all_z_img_org = all_z_img = InputG(z=all_z.cuda(), c=label.cuda(), truncation_psi=truncation_psi, noise_mode=noise_mode)
        # cur_z_org = cur_z = torch.from_numpy(interp_latents[j:j+1])
        all_z_img = img_transform(all_z_img,transform,transform_flag)
        # print(all_z_img.shape,label.shape)

        for ind in range((num_interps+1)**2):
            cur_img = G(all_z_img[ind:ind+1], label[ind:ind+1], truncation_psi=truncation_psi, noise_mode=noise_mode)
            if ind != 0:
        ...
        # all_img = G(all_z_img[0:100], label[0:100], truncation_psi=truncation_psi, noise_mode=noise_mode)


        all_img = (all_img * 127.5 + 128).clamp(0, 255).to(torch.uint8)
        all_z_img_org = (all_z_img_org * 127.5 + 128).clamp(0, 255).to(torch.uint8)


        img_grid = make_grid(all_img, nrow = num_interps+1)
        z_grid = make_grid(all_z_img_org, nrow = num_interps+1)
