...
def generate_images(
    ...
    os.makedirs(outdir+'/InterpolImages', exist_ok=True)
    os.makedirs(outdir+'/InterpolMidImages', exist_ok=True)

    impath = outdir+'/Interpol_Images'
    midpath = outdir+'/Interpol_MidImages'
    ...
    for i in range(len(seeds)-1):
        start_seed = seeds[i]
        end_seed = seeds[i+1]
        print('Interpolating on  MidImage for seeds %d-%d) ...' % (start_seed, end_seed))
        z_start = torch.from_numpy(np.random.RandomState(start_seed).randn(1, G.z_dim)).to(device)
        z_end = torch.from_numpy(np.random.RandomState(end_seed).randn(1, G.z_dim)).to(device)
        ...
        z_start_img = InputG(z=z_start, c=label, truncation_psi=truncation_psi, noise_mode=noise_mode)
        # z_start_img = img_transform(z_start_img,transform)

        z_end_img = InputG(z=z_end, c=label, truncation_psi=truncation_psi, noise_mode=noise_mode)
        

        num_interps = 10
        stack = np.vstack([z_start_img.cpu().numpy(), z_end_img.cpu().numpy()]) 
        linfit = interp1d([1,num_interps+1], stack, axis=0)
        interp_latents = linfit(list(range(1,num_interps+1)))
        
        for j in range(interp_latents.shape[0]):
            cur_z_org = cur_z = torch.from_numpy(interp_latents[j:j+1])
            cur_z = img_transform(cur_z,transform)
            img = G(cur_z, label, truncation_psi=truncation_psi, noise_mode=noise_mode)
            img = (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
            cur_z_org = (cur_z_org.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)

            PIL.Image.fromarray(cur_z_org[0].cpu().numpy(), 'RGB').save(f'{midpath}/seed{start_seed:04d}_interpol{j:02d}.png')
            PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{impath}/seed{start_seed:04d}_interpol{j:02d}.png')
